...
func CalculateBasalMetabolism(weight, height float64, age, sex int) float64 {
	if weight <= 0 || height <= 0 || age <= 0 || (sex != 0 && sex != 1) {
		return 0
	}
	...
	return (coefficient * weight * 0.48 * height * 0.50 * float64(age) - 0.13) * (1000 / 4.1855)
}
